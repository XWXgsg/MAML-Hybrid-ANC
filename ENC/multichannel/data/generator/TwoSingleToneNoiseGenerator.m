%% Title: Dual-Tone Noise Generator for ANC Simulation
%  生成双频正弦噪声信号，用于陷波器 ANC 算法验证
%
...
% 纯净双频信号
signal_clean = A1 * sin(2*pi*f1*t + phi1) + A2 * sin(2*pi*f2*t + phi2);
...
figure('Name', 'Dual-Tone Noise', 'Position', [150, 150, 1000, 600], 'Color', 'w');

% 时域
subplot(2, 2, 1);
plot(t, dn(:,1), 'b');
xlabel('Time (s)'); ylabel('Amplitude');
title(sprintf('时域波形 (f_1=%d Hz, f_2=%d Hz)', f1, f2));
...
title('功率谱密度');
grid on; xlim([0, 500]);
xline(f1, 'r--', sprintf('%d Hz', f1), 'LineWidth', 1.2, 'LabelOrientation', 'horizontal');
xline(f2, 'm--', sprintf('%d Hz', f2), 'LineWidth', 1.2, 'LabelOrientation', 'horizontal');
...
sgtitle(sprintf('双频噪声信号: f_1=%d Hz, f_2=%d Hz, fs=%d Hz, SNR=%d dB', f1, f2, fs, SNR_dB), ...
    'FontSize', 13, 'FontWeight', 'bold');

%% =================== 保存 ===================

save('sim_dual_tone.mat', 'dn', 's', 'fs', 'T', 'N', 'f1', 'f2', 'M', 'L', 'J');
fprintf('已保存 sim_dual_tone.mat\n');
fprintf('  频率: %d Hz + %d Hz | 采样率: %d Hz | 时长: %d s | SNR: %d dB\n', f1, f2, fs, T, SNR_dB);